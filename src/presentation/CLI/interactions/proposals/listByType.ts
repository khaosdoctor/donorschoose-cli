...
export async function listByType (donorsChooseService: DonorsChooseService, userService: UserService, placesService: GoogleService, logger: Logger) {
  const spinner = ora()
  try {
    ...
    const keywordAnswer = await inquirer.prompt([{
      ...
      name: 'projectType',
      choices: [
        { name: 'Student-led proposals', value: true },
        { name: 'Teacher-led proposals', value: false }
      ]
    }])

    spinner.start('Loading proposals...')
    const results = await donorsChooseService.getProposalsByType(keywordAnswer.projectType)
    spinner.stop()
